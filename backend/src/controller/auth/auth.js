...
export const generateOtp = async (req, res) => {
  ...
  try {
    ...
    console.log(error);
   return res.status(500).json({status:"fail", message: "internel server issue" });
  }
};
...
export const verifyOtp = async(req,res)=>{
  ...
  try {
    const user = await User.findOne({email})
    if(!user){
      return res.status(400).json({status:"fail",message:"user email is invalide"})
    }
    if(user.otp != otp || user.otpExpires <Date.now()){
      return res.status(400).json({status:"fail",message:"user OTP invalide "})
    }
    user.otp = null
    user.otpExpires = null
    await user.save()
    return res.status(200).json({status:"success",message:"user OTP verifyed successfully"})
  } catch (error) {
      console.log(error);
   return res.status(500).json({ message: "internel server issue" });
  }
